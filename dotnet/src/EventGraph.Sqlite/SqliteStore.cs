...
namespace EventGraph.Sqlite;

/// <summary>
/// SQLite-backed event store. Satisfies IStore. Data persists across process restarts.
...
public sealed class SqliteStore : IStore, IDisposable
{
    ...
    public SqliteStore(string connectionString = "Data Source=:memory:")
    {
    ...
    private static Event RowToEvent(SqliteDataReader r)
    {
        var causeIds = JsonSerializer.Deserialize<List<string>>(r.GetString(7))!
            .Select(c => new EventId(c)).ToList();
        var contentJson = r.GetString(5);
        var content = JsonSerializer.Deserialize<Dictionary<string, object?>>(contentJson)
            ?? new Dictionary<string, object?>();
        var sigBytes = new byte[r.GetBytes(11, 0, null, 0, 0)];
        r.GetBytes(11, 0, sigBytes, 0, sigBytes.Length);

        return new Event(
            r.GetInt32(3),          // version
            new EventId(r.GetString(1)),
            new EventType(r.GetString(2)),
            r.GetInt64(4),          // timestamp_nanos
            new ActorId(r.GetString(5) /* actually source at col 5 */),
            content,
    ...
    private Event ReadEvent(SqliteDataReader r)
    {
        var causeIds = JsonSerializer.Deserialize<List<string>>(r.GetString(7))!
            .Select(c => new EventId(c)).ToList();
    ...
    private List<Event> QueryEvents(string sql, params SqliteParameter[] parameters)
    {
        using var cmd = _conn.CreateCommand();
        cmd.CommandText = sql;
    ...
    private Event? QuerySingleEvent(string sql, params SqliteParameter[] parameters)
    {
        using var cmd = _conn.CreateCommand();
        cmd.CommandText = sql;
        foreach (var p in parameters) cmd.Parameters.Add(p);
    ...
    public Event Append(Event ev)
    {
        lock (_lock)
        {
            ...
            var existing = QuerySingleEvent(
                ...
                new SqliteParameter("@id", ev.Id.Value));
    ...
    public Event Get(EventId eventId)
    {
        lock (_lock)
        {
            return QuerySingleEvent(
                "SELECT * FROM events WHERE event_id = @id",
                new SqliteParameter("@id", eventId.Value))
    ...
    public List<Event> ByType(EventType type, int limit)
    {
        lock (_lock)
        {
            return QueryEvents(
                ...
                new SqliteParameter("@t", type.Value),
                new SqliteParameter("@lim", limit));
        }
    }
    ...
    public List<Event> BySource(ActorId source, int limit)
    {
        lock (_lock)
        {
            return QueryEvents(
                ...
                new SqliteParameter("@s", source.Value),
                new SqliteParameter("@lim", limit));
        }
    }
    ...
    public List<Event> ByConversation(ConversationId id, int limit)
    {
        lock (_lock)
        {
            return QueryEvents(
                ...
                new SqliteParameter("@c", id.Value),
                new SqliteParameter("@lim", limit));
        }
    }
    ...
    public List<Event> Ancestors(EventId id, int maxDepth)
    {
        lock (_lock)
        {
            var start = QuerySingleEvent(
                "SELECT * FROM events WHERE event_id = @id",
                new SqliteParameter("@id", id.Value))
