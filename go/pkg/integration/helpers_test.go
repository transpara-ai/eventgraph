...
func (e *testEnv) registerActor(name string, pkByte byte, actorType event.ActorType) actor.IActor {
	e.t.Helper()
...
func containsEventType(events []event.Event, typeName string) bool {
	for _, ev := range events {
		if ev.Type().Value() == typeName {
			return true
		}
	}
	return false
