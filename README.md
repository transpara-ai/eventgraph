...
> **3,278 tests across 5 languages.** Go reference implementation with 201 primitives, 28 agent primitives, Code Graph spec language (35 event types, 7 compositions), intelligence providers (Anthropic, OpenAI-compatible, Claude CLI), EGIP protocol, 21 integration scenarios, 13 composition grammars. Language packages for Rust, Python, TypeScript, and .NET — all conformance-tested against Go reference hashes. Database stores (SQLite, Postgres, MySQL, SQL Server). Published to npm, PyPI, NuGet, and crates.io.
...
| 11 | Code Graph spec language (35 event types, 7 compositions — all 5 languages) | **Done** |
